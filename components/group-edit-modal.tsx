...
export function GroupEditModal({ isOpen, onClose, onSave, group, users, currentUser }: GroupEditModalProps) {
  ...
  const [contactPersons, setContactPersons] = useState(
    group.contactPersons || [{ name: "", designation: "", phone: "", email: "", isPrimary: true }],
  )
  ...
  useEffect(() => {
    ...
    setContactPersons(group.contactPersons || [{ name: "", designation: "", phone: "", email: "", isPrimary: true }])
  }, [group])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updatedGroupData: Partial<StoreGroup> = {
        ...
        keyAccountManager,
        keyAccountManagerName: users.find((u) => u.id === keyAccountManager)?.name || "",
        contactPersons,
      }
  ...
  const handleAddContact = () => {
    setContactPersons([...contactPersons, { name: "", designation: "", phone: "", email: "", isPrimary: false }])
  }

  const handleContactChange = (index: number, field: keyof typeof contactPersons[0], value: string | boolean) => {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <Label htmlFor="keyAccountManager">Key Account Manager</Label>
            <Select value={keyAccountManager} onValueChange={setKeyAccountManager}>
              <SelectTrigger id="keyAccountManager">
