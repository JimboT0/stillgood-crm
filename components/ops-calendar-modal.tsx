...
const addToCalendar = (store: StoreOpsView, type: "training" | "launch") => {
    const eventDate = type === "training" ? store.trainingDate : store.launchDate;
    if (!eventDate) {
        console.error(`No ${type} date provided for store: ${store.tradingName || store.storeId}`);
        return;
    }

    const jsDate = eventDate instanceof Timestamp ? eventDate.toDate() : new Date(eventDate);
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <Button
                                            ...
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                addToCalendar(event.resource.store, event.resource.type);
                                            }}
                                    ...
                                    <div key={index} className="border rounded-lg p-4 space-y-4">
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm text-gray-600">
                                                    {product.retailPrice ? `R${product.retailPrice.toFixed(2)}` : "N/A"}
                                                </p>
                                            </div>
                                            <div>
                                                <p className="text-sm font-medium">Estimated Value (R)</p>
                                                <p className="text-sm text-gray-600">
                                                    {product.estimatedValue ? `R${product.estimatedValue.toFixed(2)}` : "N/A"}
                                                </p>
                                            </div>
                                        </div>
                                        <div>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <Button
                                                variant="outline"
                                                className="text-xs bg-transparent"
                                                onClick={() => addToCalendar(event.resource.store, event.resource.type)}
                                                aria-label={`Set reminder for ${event.title}`}
                                            >
